...
def insert_record():
    ...
    if register_request.get() == "":
        warn = "What is your request?"
    else:
        check_counter += 1
    ...
    if check_counter == 9:        
        try:
            con = sqlite3.connect('userdata.db')
            cur = con.cursor()
            cur.execute("INSERT INTO record VALUES (:name, :email, :contact, :gender, :country, :password, :request)", {
...
countries = []
variable = StringVar()
world = open("C:/Users/konan/internship/projects/demo_project/countries.txt", 'r')
...
register_request.grid(row=5, column=1, pady=10, padx=20)
